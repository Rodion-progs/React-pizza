...
import { clearCart, removeCartItem, decreaseCartItem, increaseCartItem } from '../redux/actions/cart';
import { Link } from 'react-router-dom';

function Cart() {
  const dispatch = useDispatch();
  const { totalPrice, totalCount, items } = useSelector(({cart}) => cart);
  const AddedPizzas = Object.values(items).map(item => item.items[0])
        ...
        totalCount ?
          <div className="cart">
            ...
            <div className="content__items">
              {AddedPizzas && AddedPizzas.map((cartItem) =>
                <CartItem
                  name={cartItem.name}
                  type={cartItem.type}
                  size={cartItem.size}
                  totalPrice={items[cartItem.id].totalPrice}
                  totalItems={items[cartItem.id].items.length}
                  key={cartItem.id}
                  id={cartItem.id}
